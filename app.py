...
def get_activities():
  ...
  if df.empty: return {}
  return {row["act_id"]: {"name": row["name"], "time": row["time"], "color": row["color"] if row["color"] else "藍色"} for _, row in df.iterrows()}
...
def get_schedule():
  ...
  sched = {}
  if df.empty: return sched
  for _, row in df.iterrows():
    d = date.fromisoformat(row["date"])
    if d not in sched: sched[d] = []
    sched[d].append({"item_id": row["item_id"], "act_id": row["act_id"], "name": row["name"], "time": row["time"], "color": row["color"] if row["color"] else "藍色"})
  return sched
...
with col_calendar:
  ...
  end_date_display = st.session_state.start_week_date + timedelta(days=(10 * 7) - 1)
  c_title.markdown(f"<h3 style='text-align: center; margin: 0; color: #1e293b; font-size: 15px; padding-top: 6px;'>{st.session_state.start_week_date.strftime('%Y/%m/%d')} ~ {end_date_display.strftime('%Y/%m/%d')}</h3>", unsafe_allow_html=True)
  ...
  for idx, d_name in enumerate(week_days):
    h_cols[idx + 1].markdown(f"<div style='font-weight: bold; text-align: center; background-color: #f1f5f9; padding: 6px; border-radius: 4px; color: #334155; font-size: 12px;'>{d_name}</div>", unsafe_allow_html=True)
  ...
  for w in range(10):
    ...
    with row_cols[0]:
      if current_month != last_month:
        st.markdown(f"<div style='min-height: 90px; display: flex; flex-direction: column; align-items: center; justify-content: center; font-weight: bold; color: #0284c7; background-color: #f0f9ff; border-left: 3px solid #0284c7; border-radius: 4px; font-size: 12px; text-align: center;'>🌸<br>{current_month}月</div>", unsafe_allow_html=True)
        last_month = current_month
      else:
        st.markdown("<div style='min-height: 90px; border-left: 3px solid #e0f2fe;'></div>", unsafe_allow_html=True)
        ...
        with st.container(border=True):
          ...
          if day_events:
            for ev in day_events:
              ...
              with c_del:
                if st.button("✕", key=f"del_d_{current_d.isoformat()}_{ev['item_id']}"):
                  delete_schedule_db(ev["item_id"])
                  st.rerun()
              rendered_count += 1
          while rendered_count < 2:
            st.caption("<span style='opacity:0; user-select:none;'>-</span>", unsafe_allow_html=True)
            rendered_count += 1
...
with col_setting:
  ...
  if activities and st.session_state.get("selected_act_id") in activities:
    ...
    if c_btn1.button("📅 單次", use_container_width=True):
      ...
      add_schedule_db(item_id, st.session_state.selected_act_id, st.session_state.sel_date.isoformat(), chosen_info["name"], chosen_info["time"], chosen_info["color"])
      st.success("成功安排！")
      st.rerun()
    ...
    if c_btn2.button("🔄 4週", use_container_width=True):
      for i in range(4):
        ...
        item_id = str(uuid.uuid4())[:8]
        add_schedule_db(item_id, st.session_state.selected_act_id, target_date.isoformat(), chosen_info["name"], chosen_info["time"], chosen_info["color"])
      st.success("成功新增4週！")
      st.rerun()
  ...
  act_color = st.selectbox("選擇標籤顏色", ["粉紅", "藍色", "紫色", "綠色", "黃色"])
