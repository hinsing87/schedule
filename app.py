...
# 自訂 CSS：左邊放月曆（支援手機橫向滾動），右邊放設定
st.markdown(
    """
    <style>
    /* 強制月曆區塊在手機上可橫向滾動，並且 8 欄不自動換行 */
...
with col_calendar:
  ...
  # 使用純 HTML 表格確保手機橫向滾動穩定、電腦版完美對齊
  table_html = (
      '<div'
      ' class="calendar-scroll-container"><table'
  ...
  while i < 10:
    ...
    for row_idx in range(i, j):
      ...
      if row_idx == i:
        table_html += (
            f"<td class='month-col' rowspan='{count}'>🌸<br>{m}月</td>"
        )

      for d_idx in range(7):
        curr_d = week_start_d + timedelta(days=d_idx)
        day_events = schedule.get(curr_d, [])

        table_html += f"<td><div style='font-size: 13px; font-weight: bold; color: #475569; margin-bottom: 4px;'>{curr_d.month}/{curr_d.day}</div>"

        if day_events:
          for ev in day_events:
            emoji = color_emojis.get(ev["color"], "📌")
            # 每一項活動顯示名稱與刪除按鈕
            table_html += f"<div style='font-size: 12px; margin-bottom: 2px; color: #1e293b;'>{emoji} {ev['name']} ({ev['time']})</div>"
        else:
          table_html += (
              "<div"
              " style='color: transparent; font-size:"
              " 11px; user-select: none;'>-</div>"
          )
