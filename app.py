...
with col_calendar:
  ...
  # 星期標題列
  st.markdown(
      '<div style="overflow-x: auto; white-space: nowrap; padding-bottom:'
      ' 6px;">',
      unsafe_allow_html=True,
  )
  ...
  for w in range(10):
    ...
    with row_cols[0]:
      if current_month != last_month:
        st.markdown(
            f"<div style='min-height: 120px; display: flex; flex-direction:"
        ...
        st.markdown(
            "<div style='min-height: 120px; border-left: 3px solid #e0f2fe;'>"
            "</div>",
            unsafe_allow_html=True,
        )
    ...
    for i in range(7):
      ...
      with row_cols[i + 1]:
        with st.container(border=True):
          st.markdown(
              f"<div style='font-size: 12px; font-weight: bold; color:"
  ...
  st.markdown(
      ...
      " margin-top: 8px;'>💡 手機瀏覽時可左右滑動查看完整 7 日日曆</p>",
      unsafe_allow_html=True,
  )
